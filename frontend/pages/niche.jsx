...
function badge(text, type){
  ...
  return <span className={cls} style={{marginRight:6}}>{text}</span>;
}
...
export default function Niche(){
  ...
  function demandBadge(val){
    if(val==='—') return badge('Demand —','warn');
    const v = (''+val).toLowerCase();
    if(v.includes('high')) return badge('Demand 🟢 '+val,'ok');
    if(v.includes('low')) return badge('Demand 🔴 '+val,'err');
    return badge('Demand '+val,'warn');
  }
  function compBadge(val){
    if(val==='—') return badge('Comp —','warn');
    const v = (''+val).toLowerCase();
    if(v.includes('low')) return badge('Comp 🟢 '+val,'ok');
    if(v.includes('high')) return badge('Comp 🔴 '+val,'err');
    return badge('Comp '+val,'warn');
  }
