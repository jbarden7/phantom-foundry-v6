    ...
    <>
      ...
      {open && (
        <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,.45)', zIndex:1000}} onClick={()=>setOpen(false)}>
          <aside style={{position:'absolute', top:0, right:0, width:'360px', height:'100%', background:'var(--panel)', borderLeft:'1px solid var(--border)', padding:'20px', overflowY:'auto', boxShadow:'-20px 0 60px rgba(0,0,0,.35)'}} onClick={e=>e.stopPropagation()}>
            ...
            <div className="row" style={{alignItems:'center'}}>
              ...
              <button className="btn ghost" onClick={toggleTheme}>{theme === 'light' ? 'Switch to Dark' : 'Switch to Light'}</button>
            </div>
            <p className="help">These preferences are saved locally in your browser.</p>
